...
class Response:
    ...
    def from_status_code(cls, status_code: int):
        """Create an HTTP response from a valid status code.

        Raises:
            ValueError: When the status code is not a valid http.HTTPStatus,
            see available status codes from IANA-registered.
        """
        http_status = HTTPStatus(status_code)
        status = f"{http_status.value} {http_status.phrase}"
        return cls(status=status, content=http_status.phrase)
